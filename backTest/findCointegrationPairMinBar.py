...
def loadBarDataFun():
	...
	for root, folders, files in list_dirs:
		for f in files:
			dataDate = float(f[:8])
			if dataDate >= 20130924 and dataDate <= 20131231:
...
#筛选配对股票
def stockPairSelectFun(barData):
	global Results
			...
			if isInTheSamePlateFun(stock1, stock2):	#如果在同一个板块
				...
				if stock1 != stock2 and not Results.has_key(key):
					if stock1 in StockPrice and stock2 in StockPrice:
						beta, isCointegration, MEAN, STD, _open, _close, _stopLoss = fittingSeriesFun(series)
						...
						if "beta < 1.3 and beta > 0.7" and STD < 0.03:
							...
							logFile.write(content)
							logFile.close()
							Results[key] = (beta, MEAN, STD, _open, _close, _stopLoss, isCointegration)
	pass
...
def fittingSeriesFun(series):
	...
	y = ADF_Series.C_ADF_Series(False,x)
	St = ADF_Series.C_ADF_Series(False,x)
...
def main():
	...
	#筛选股票对
	stockPairSelectFun(barData)

	#dataDict = {}

	#pool = multiprocessing.Pool(processes= int(multiprocessing.cpu_count()*0.4))
	#for stock in RZRQ:
	#	if barData.has_key(stock):
	#		data = sorted(barData[stock].iteritems(), key=lambda d:d[0])
	#		creatADFSeriesNewFun(stock, dataDict, zip(*data)[1])
	#		#result = pool.apply_async(creatADFSeriesNewFun, (stock, dataDict, zip(*data)[1]))
	#pool.close()
	#pool.join()
	#print len(dataDict)
