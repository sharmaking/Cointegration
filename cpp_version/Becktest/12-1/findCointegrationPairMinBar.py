...
import os, csv, multiprocessing, ctypes
...
class MyBeta_t(ctypes.Structure):
	_fields_ = [ ("arrF3", ctypes.c_double * 8),
		("arrF2", ctypes.c_double * 7),
		("arrF1", ctypes.c_double * 6)]
              
class MyResult_t(ctypes.Structure):
	_fields_ = [ ("iOrder", ctypes.c_int),
		("iFulfillStep", ctypes.c_int),
		("arrBeta", MyBeta_t * 11)]
...
def loadBarDataFun():
	...
	for root, folders, files in list_dirs:
		for f in files:
			dataDate = float(f[:8])
			if dataDate >= 20131201 and dataDate <= 20140201:
